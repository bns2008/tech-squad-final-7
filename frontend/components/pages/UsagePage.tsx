  ...
  return (
    <div className="flex items-center gap-4 py-4 border-b border-[var(--border)] last:border-none">
      <div className="flex-shrink-0">
        <Ring pct={pct} color={isFull ? "#ef4444" : isHigh ? "#f59e0b" : color} />
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          <Icon size={13} className="text-[var(--text-muted)]" />
          <span className="text-sm font-bold text-[var(--text)]">{label}</span>
          {isFull && <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-red-100 text-red-600 dark:bg-red-500/10 dark:text-red-400">FULL</span>}
          {isHigh && !isFull && <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-600 dark:bg-amber-500/10 dark:text-amber-400">HIGH</span>}
        </div>
        {desc && <p className="text-[11px] text-[var(--text-subtle)] mb-1.5">{desc}</p>}
        <div className="flex items-center gap-2">
          ...
          <span className={cn("text-xs font-bold tabular-nums flex-shrink-0", isFull ? "text-red-500" : isHigh ? "text-amber-500" : "text-[var(--text-muted)]")}>
            {used} / {total}
          </span>
          <span className="text-[11px] text-[var(--text-subtle)] flex-shrink-0">({pct}%)</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="card p-4 flex items-start gap-3">
      ...
      <div className="min-w-0">
        <div className="text-xl font-bold text-[var(--text)] tabular-nums leading-tight">{value}</div>
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-8">
      ...
      <motion.div {...fadeUp(0)} className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <div className="flex items-center gap-2.5 mb-1">
            ...
            <h1 className="text-2xl font-bold text-[var(--text)]">Usage & Limits</h1>
          </div>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            ...
            {isPro && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-amber-200 dark:bg-amber-500/20 text-amber-700 dark:text-amber-400">ACTIVE</span>}
          </div>
        ...
        <div className="text-right flex-shrink-0">
          ...
          <div className="text-xs text-[var(--text-muted)]">{isPro ? "per month" : "forever"}</div>
        </div>
      </motion.div>

      {/* ── Summary stats ───────────────────────────────────────────────────── */}
      <motion.div {...fadeUp(0.08)}>
        <h2 className="text-xs font-bold uppercase tracking-widest text-[var(--text-subtle)] mb-3 px-1">Summary</h2>
      ...
      <motion.div {...fadeUp(0.12)}>
        <h2 className="text-xs font-bold uppercase tracking-widest text-[var(--text-subtle)] mb-3 px-1">Plan Quotas</h2>
        ...
        {!isPro && (
          <motion.div {...fadeUp(0.16)}
            ...
            <button onClick={() => onNavigate("pricing")}
              className="flex items-center gap-1 text-xs font-bold text-[var(--primary)] hover:underline flex-shrink-0">
      ...
      <motion.div {...fadeUp(0.15)}>
        <h2 className="text-xs font-bold uppercase tracking-widest text-[var(--text-subtle)] mb-3 px-1">Billing Cycle</h2>
        <div className="card p-5 grid grid-cols-1 sm:grid-cols-3 gap-4 divide-y sm:divide-y-0 sm:divide-x divide-[var(--border)]">
          {[
            ...
            <div key={label} className="flex items-center gap-3 py-2 sm:py-0 sm:px-4 first:pl-0 last:pr-0">
              ...
              <div>
                <p className="text-[11px] text-[var(--text-subtle)]">{label}</p>
      ...
      <motion.div {...fadeUp(0.18)}>
        <h2 className="text-xs font-bold uppercase tracking-widest text-[var(--text-subtle)] mb-3 px-1">Feature Access</h2>
        <div className="card overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--border)]" style={{ background: "var(--surface)" }}>
                <th className="text-left px-5 py-3 text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">Feature</th>
                <th className="text-center px-4 py-3 text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">Your Plan</th>
                <th className="text-center px-4 py-3 text-xs font-bold text-amber-500 uppercase tracking-wider hidden sm:table-cell">Pro</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[var(--border)]">
              {[
                ...
                const hasIt = typeof yours === "boolean" ? yours : true;
                return (
                  <tr key={feature} className="hover:bg-[var(--surface)] transition-colors">
                    ...
                    <td className="px-4 py-3 text-center hidden sm:table-cell">
                      <span className="text-sm font-bold text-amber-500">{pStr}</span>
                    </td>
                  </tr>
                );
              })}
      ...
      {projectStats.length > 0 && (
        <motion.div {...fadeUp(0.22)}>
          <h2 className="text-xs font-bold uppercase tracking-widest text-[var(--text-subtle)] mb-3 px-1">Projects Breakdown</h2>
          <div className="card overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[480px]">
                <thead>
                  <tr className="border-b border-[var(--border)]" style={{ background: "var(--surface)" }}>
                    <th className="text-left px-5 py-3 text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">Project</th>
                    <th className="text-center px-4 py-3 text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">Files</th>
                    <th className="text-center px-4 py-3 text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">Done</th>
                    <th className="text-center px-4 py-3 text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">Failed</th>
                    <th className="text-center px-4 py-3 text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider hidden sm:table-cell">DB</th>
                    <th className="text-right px-5 py-3 text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider hidden md:table-cell">Updated</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[var(--border)]">
                  {projectStats.map((p) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-center text-[var(--text-muted)] font-medium">{p.total}</td>
                        <td className="px-4 py-3 text-center">
                        ...
                        <td className="px-4 py-3 text-center hidden sm:table-cell">
                          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[var(--surface)] text-[var(--text-muted)] border border-[var(--border)]">
                            {p.dbType.toUpperCase()}
                          </span>
                        </td>
                        <td className="px-5 py-3 text-right text-xs text-[var(--text-subtle)] hidden md:table-cell">
                          {timeAgo(p.updatedAt)}
                        </td>
                      </tr>
                    );
      ...
      {quickHistory.length > 0 && (
        <motion.div {...fadeUp(0.26)}>
          <h2 className="text-xs font-bold uppercase tracking-widest text-[var(--text-subtle)] mb-3 px-1">Recent Quick Converts</h2>
          <div className="card overflow-hidden">
            <div className="divide-y divide-[var(--border)]">
              {quickHistory.slice(0, 8).map((r) => (
                <div key={r.id} className="flex items-center gap-3 px-5 py-3">
                  <div className="w-7 h-7 rounded-lg bg-sky-50 dark:bg-sky-500/10 flex items-center justify-center flex-shrink-0">
                    <Sparkles size={12} className="text-sky-600" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-[var(--text)] truncate">{r.filename}</p>
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <p className="text-[10px] text-emerald-500 font-semibold">Done</p>
                  </div>
                </div>
              ))}
            </div>
            {quickHistory.length > 8 && (
              <div className="px-5 py-3 border-t border-[var(--border)]">
                <button onClick={() => onNavigate("history")}
                  ...
                  View all {quickHistory.length} entries <ArrowRight size={11} />
                </button>
              </div>
            )}
          </div>
